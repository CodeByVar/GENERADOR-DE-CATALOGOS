...
"""
  ...
  <script>
    ...
    async function fetchLiveStock() {{
      ...
      // 2. CONSULTA EN SEGUNDO PLANO CON TIMEOUT SEGURO (28s)
      try {{
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 28000);
        
        let res = null;
        // Intento 1: Llamar al endpoint /api/stock (servidor local Python o Vercel Serverless)
        try {{
        ...
        // Intento 2: Si /api/stock falló (ej: abriendo como archivo local file:///), consultar directo a Google Apps Script
        if (!res || !res.ok) {{
